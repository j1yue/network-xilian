...
　　[Gephi](https://gephi.org/) 是一款开源、免费、跨平台的网络分析软件。和 Python 相比，Gephi 有简单直观的图形界面，非常便于我们学习和使用。Gephi 的运行需要 [Java](https://www.java.com/zh_CN/) 环境，下载安装即可，在此不再赘述。其下面简要介绍 Gephi 的操作方法。
...
　　**2.2.2** 但更多时候，我们只有原始数据，而需要软件为我们生成 `.gexf` 等网络图的格式，这时 Gephi 也能很好地满足我们的需要。导入的数据最好是 `.csv` 或 `.tsv` 格式，可以通过 Excel 另存为得到，也可以直接用文本编辑器（参见 [§1.1.2](#1.1-准备工作)）编辑。
...
其中 `Id` 是必不可少的，`Label` 处于查看的方便，可以设置为汉字；`timeset` 在静态图中可以删去，而 `modularity_class` 则是结点的度，可以在软件中自动生成，不必手动填写。
